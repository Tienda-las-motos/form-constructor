...
export class FormConstructorService {
  ...
  constructor(private fs: AngularFirestore, private _snack: MatSnackBar) {}

  async callFormByName(
    collection: string,
    formName: string
  ): Promise<iFormResult> {
    ...
    const inputsArray = await formRef
      .collection('inputs')
      .orderBy('index')
      .get();
    var inputs: any[] = [];
    inputsArray.forEach(async (input) => {
      await inputs.push(input.data());
    });
  ...
  async saveForm(form: FormModel, inputs: InputModel[]) {
    ...
    let formCreated = await (
      await collRef.where('nombre', '==', form.nombre).get()
    ).docs[0];
  ...
  private async _saveInputs(inputs: any[], fid: string, collection: string) {
    const inputsColRef = this.fs.collection(`${collection}/${fid}/inputs`).ref;
